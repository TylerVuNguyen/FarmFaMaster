...
class SuaKhuVuonActivity : AppCompatActivity() {
    ...
    private fun updateGarden() {
        ...
        if(checkGardenName && checkGardenImage){
            var garden = Garden(garden_id, garden_name,image,formatted)
...
//                val bitmap = BitmapFactory.decodeStream(inputStream)
    ...
    private fun initDataEdit() {
        val garden_id: Int = getDataFromItent()

        // gán lại id để tý update data
        var garden: Garden = getGardenById(garden_id)
        ...
        this.selected_garden_image_edit.setImageBitmap(bitmap)
