...
class DeviceAdapter(private var activity: Activity, private var items: ArrayList<Device>) :  BaseAdapter(){
    ...
    private class ViewHolder(row: View?) {
        ...
        init {
            this.device_name = row?.findViewById<TextView>(R.id.name_device)
            this.img_device = row?.findViewById<ImageView>(R.id.img_device)
        }
    }
    override fun getView(position: Int, convertView: View?, parent: ViewGroup): View {
        ...
        viewHolder.img_device?.setImageResource(device.device_photo!!)
